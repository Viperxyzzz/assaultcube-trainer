...
DWORD WINAPI GDI_drawing::esp(entityList entities, LocalPlayer player,Vector3 screen)
{
	...
	while (true)
	{
		...
		for (int i = 0; i < entities.totalPlayers; i++)
		{
			...
			if (WorldToScreen(entities.list[i].feetPosition, screen, player.viewMatrix, 800, 600) && entities.list[i].health > 0)
			{
				if (entities.list[i].team != player.team)
				{
					GDI_drawing::DrawESP(screen.x, screen.y, GetDistance3D(player.feetPosition, entities.list[i].feetPosition), entities.list[i].health, entities.list[i].name, GDI_drawing::hBrushEnemy, GDI_drawing::enemyColor);
				}
			}
		}
	}
